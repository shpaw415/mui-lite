...
//TODO: MuiBreadcrumbs, MuiRating, MuiAutocomplete, AlertProps
...
export interface Localization {
	components?: {
		...
		MuiRating?: {
			defaultProps: Pick<
				{ emptyLabelText?: string; getLabelText: (value: number) => string },
		...
		MuiPagination?: {
			defaultProps: Pick<PaginationProps, "aria-label" | "getItemAriaLabel">;
		};
	};
}
...
export const kuCKB: Localization = {
	components: {
		...
		MuiRating: {
			defaultProps: {
				getLabelText: (value) =>
					`${value} ${value !== 1 ? "ئەستێرەکان" : "ئەستێرە"}`,
	...
	components: {
		...
		MuiTablePagination: {
			defaultProps: {
				...
				labelDisplayedRows: ({ from, to, count }) =>
					`${from}–${to} daripada ${
						count !== -1 ? count : `lebih daripada ${to}`
					}`,
			},
		},
		MuiRating: {
	...
	components: {
		...
		MuiTablePagination: {
			defaultProps: {
				...
				labelDisplayedRows: ({ from, to, count }) =>
					`${
						count !== -1 ? `${count} میں سے` : `${to} سے ذیادہ میں سے`
					} ${from} سے ${to} قطاریں`,
			},
		},
		MuiRating: {
