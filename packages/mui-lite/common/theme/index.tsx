...
export const MuiIds: string[] = [];
export const MuiStyleVariables: {
	id: string;
...
/** @deprecated Prefer string palette paths or token objects via resolveColorValue */
...
/** MUI-compatible sx prop (system shorthands, responsive, callbacks, nested selectors) */
export type SxProps = SxPropsBase<MuiTheme | ThemeLike>;
...
export class MuiBaseStyleUtils<Variant, suffixesType> {
	static extractColorToRGB(colorData: string): number[] {
		if (colorData.startsWith("#")) {
			const rgb = MuiBaseStyleUtils.hexToRgb(colorData);
			if (!rgb) throw new Error(`${colorData} is not a valid Hex color format`);
			return rgb;
		}
		const maybeRGBA = MuiBaseStyleUtils.extractCssRGB(colorData);
...
/** Resolve sx to inline CSS properties only (media/pseudos require classNameFromSx via useStyle). */
export function sxToStyle(
	theme: MuiTheme,
	sxProps?: SxProps,
): Partial<React.CSSProperties> {
	return resolveSx(theme, sxProps).style;
}
...
export function useStyle(sxProps?: SxProps, apply?: Partial<ApplyStyle>) {
	...
	const resolved = useMemo(() => {
		const result = resolveSx(theme, sxProps);
		return {
			styleFromSx: StyleApplier(result.style as Partial<CSSProperties>, apply),
			classNameFromSx: result.className ?? "",
		};
	}, [sxProps, theme, apply]);

	return {
		theme,
		styleFromSx: resolved.styleFromSx,
		classNameFromSx: resolved.classNameFromSx,
	};
}
